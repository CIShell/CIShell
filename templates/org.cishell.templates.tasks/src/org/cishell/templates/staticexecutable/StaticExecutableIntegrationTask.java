...
public class StaticExecutableIntegrationTask extends Task {
    ...
    protected void addAlgorithm(File dir) throws IOException {
        ...
        if (serviceFile != null && configFile != null) {
            ...
            replace.setFile(componentFile);
            replace.setToken("${service.properties}");
            
            String algName = serviceFile.getParentFile().getName();
            replace.setValue(algName+"/"+serviceFile.getName());
            replace.execute();
            
            replace = new Replace();
            replace.setProject(getProject());
            replace.setFile(componentFile);
            replace.setToken("${component.id}");
            
            replace.setValue(symbolicName+"." + algName + ".component");
            replace.execute();
            
            replace = new Replace();
            replace.setProject(getProject());
            replace.setFile(componentFile);
            replace.setToken("${alg.dir}");
            
            replace.setValue(algName);
            replace.execute();
